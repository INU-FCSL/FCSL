...
# ======= 수직 리프트 파라미터 =======
# 주의: REL_FRAME_INDEX는 실제 +Z=위가 되는 프레임으로 설정(환경에 따라 Base=0일 수 있음)
REL_FRAME_INDEX = 2
REL_SPEED = 350     # mm/s
REL_ACC   = 350     # mm/s^2
LIFT_DZ   = 200.0   # mm
...
def main():
    print("=== 인터랙티브 Pick & Place (두 입력 후 일괄 실행) ===")
    print("안내: 먼저 Pick 번호를 고르고, 다음에 Place 번호를 고른 뒤 한 번에 실행됩니다.")
    print("종료하려면 언제든 0을 입력하세요.\n")
    ...
    try:
        while True:
            ...
            # (3) 여기서부터 '둘 다 유효'하므로, 이 시점에만 동작 시작
            p_idx = int(pick_in)
            q_idx = int(place_in)
            print(f"[INFO] 실행: Pick {p_idx} → Place {q_idx}")
